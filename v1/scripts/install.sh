...
ExecStart=${APP_DIR}/.venv/bin/gunicorn --workers 2 --worker-class gthread --threads 8 --timeout 120 --graceful-timeout 30 --keep-alive 2 --access-logfile - --error-logfile - --bind 0.0.0.0:${WEB_PUBLIC_PORT} wsgi:app
Restart=always
RestartSec=3
User=root
Group=root
